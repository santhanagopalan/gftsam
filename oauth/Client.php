...
class Client
{
    ...
    */
    ...
    const HTTP_FORM_CONTENT_TYPE_APPLICATION_JSON = 2;
     ...
     * @return array Array of parameters required by the grant_type (CF SPEC)
     */
    public function getZohoAccessToken($token_endpoint, $grant_type, array $parameters, array $extra_headers = array())
    {
        if (!$grant_type) {
            throw new InvalidArgumentException('The grant_type is mandatory.', InvalidArgumentException::INVALID_GRANT_TYPE);
        }
        ...
        if ($this->access_token) {
            switch ($this->access_token_type) {
                ...
                case self::ACCESS_TOKEN_OAUTH:
                    $http_headers['Authorization'] = 'Zoho-oauthtoken ' . $this->access_token;
                    break;
    ...
    private function executeRequest($url, $parameters = array(), $http_method = self::HTTP_METHOD_GET, array $http_headers = null, $form_content_type = self::HTTP_FORM_CONTENT_TYPE_MULTIPART)
    {
        ...
        switch($http_method) {
			...
			case self::HTTP_METHOD_PATCH:
                ...
                } else if(is_array($parameters) && self::HTTP_FORM_CONTENT_TYPE_APPLICATION_JSON === $form_content_type) {
        ...
        //Added on Nov 17, 2016 for SSL Connect Error
        ...
        if ($curl_error = curl_error($ch)) {
        	error_log("Zoho chat Exception: ".$url);
